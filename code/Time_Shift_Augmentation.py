...
def time_shift(ts ,sizes):
    beta= np.random.uniform(-1,1, 1)
    return np.array(ts)+beta, sizes
...
def image_augmentor(file_path, th_min, th_max, loop = 1):
    ...
    for _ in range(loop):
        for img in data:
            ...
            ts, size = time_shift(ts, size)
            time_shifted_img = session_2d_histogram(ts, size, img_size)
            out.append([time_shifted_img])   
    out = np.array(out)
    file_name_new = file_path[:-4] + '_time_shifted.npy'
