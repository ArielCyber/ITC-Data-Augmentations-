...
def change_RTT(ts, sizes):
    alpha= np.random.uniform(0.5,1.5, 1)
    return np.array(ts)*alpha, sizes
...
def image_augmentor(file_path, th_min, th_max, loop = 1):
    ...
    for _ in range(loop):
        for img in data:
            ...
            ts, size = change_RTT(ts, size)
            changed_rtt_img = session_2d_histogram(ts, size, img_size)
            out.append([changed_rtt_img])   
    out = np.array(out)
    file_name_new = file_path[:-4] + '_rtt.npy'
